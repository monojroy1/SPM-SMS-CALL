...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/TheSpeedX/SPM-SMS-CALL/archive/dev.zip"
        dir_name = "SPM-SMS-CALL-dev"
    else:
        zip_url = "https://github.com/TheSpeedX/SPM-SMS-CALL/archive/master.zip"
        dir_name = "SPM-SMS-CALL-master"
    ...
    if success:
        mesgdcrt.SuccessMessage("SPM-SMS-CALL was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update SPM-SMS-CALL.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/TheSpeedX/SPM-SMS-CALL.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/TheSpeedX/SPM-SMS-CALL.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/TheSpeedX/SPM-SMS-CALL/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("SPM-SMS-CALL is up-to-date")
        mesgdcrt.GeneralMessage("Starting SPM-SMS-CALL")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/TheSpeedX/SPM-SMS-CALL/raw/dev/.notify"
        else:
            url = "https://github.com/TheSpeedX/SPM-SMS-CALL/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("SPM-SMS-CALL was created by SpeedX")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("SPM-SMS-CALL will work only in Python v3")
    sys.exit()
...
description = """SPM-SMS-CALL - Your Friendly Spammer Application

SPM-SMS-CALL can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

SPM-SMS-CALL is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by SpeedX !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start SPM-SMS-CALL with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start SPM-SMS-CALL with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start SPM-SMS-CALL with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update SPM-SMS-CALL")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current SPM-SMS-CALL contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current SPM-SMS-CALL version")
